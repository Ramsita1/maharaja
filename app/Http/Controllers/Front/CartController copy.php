...
class CartController extends Controller
{
    ...
    {        
        ...
        $temp_cart=TempCartTable::where('user_id', 21)->where('store_id', $store_id)->get()->first();
    ...
    {  
        ...
        $add_to_cart_data=TempCartTable::where('user_id', 21)->where('store_id', $store_id)->get()->first();
        if($add_to_cart_data)
        {
        ...
        //echo $temp_cart;
        if($temp_cart)
          $cartData=json_decode($temp_cart->attributes,true); //json_decode($temp_cart->attributes);
         // echo $cartData;
        if (empty($cartData)) {
            Session::put('delivery_pickup_address', []);
            Session::save();
            ...
            if ($item_page == 'checkoutPage') {
                ...
                $cartHtml = getCartHtml(true,$cartData);
            }
            if ($item_page == 'cartPage' || $item_page == 'checkoutPage') {
                $response = [
                    'count' => count($cartData),
