...
<html lang="<?php echo e(str_replace('_', '-', app()->getLocale())); ?>">
   ...
   <link rel="stylesheet" type="text/css" href="<?php echo publicPath() ?>/front/slick/slick.css">
     <link rel="stylesheet" type="text/css" href="<?php echo publicPath() ?>/front/slick/slick-theme.css">
   <link rel="stylesheet" href="<?php echo publicPath() ?>/front/css/styleorder.css">
   ...
   <script src="<?php echo publicPath() ?>/front/slick/slick.js" type="text/javascript" charset="utf-8"></script>
   ...
   <div id="colorlib-page">
            ...
            <div class="row">
               <div class="col-md-12">
                  ...
                  <div class="order-sec">
                     ...
                     <?php
                     if (isset($itemPage)) {
                      if (isset($requestPerameters['pickup_when']) && $requestPerameters['pickup_when'] == 'Now')
                      {
                        ?>
                        <p><u>ASAP</u>
                        <?php
                      } elseif (isset($requestPerameters['pickup_when']) && $requestPerameters['pickup_when'] == 'Later')
                      {
                        ?>
                        <p><u><?php echo date('h:i A', strtotime($requestPerameters['order_time'])) ?></u>
                        <?php
                      }
                      if (isset($requestPerameters['order_type']) && $requestPerameters['order_type'] == 'Pickup')
                      {
                        if ($store) {
                          ?>
                           From <u><?php echo $store->store_city.' '.$store->store_postalCode ?></u></p>
                          <?php
                        }
                      }elseif (isset($requestPerameters['order_type']) && $requestPerameters['order_type'] == 'Delivery')
                      {
                        ?>
                         To <u><?php echo $requestPerameters['city'].' '.$requestPerameters['pincode'] ?></u></p>
                        <?php
                      }
                     }
                     ?>
                  ...
                  /*if (isset($settings['header']['cart_icon']) && $settings['header']['cart_icon'] == 'yes') {
                    ?>
                    <div class="cart-blk">
                      <a href="<?php echo url('cart') ?>" class="cartButton">
                        <img src="<?php echo publicPath() ?>/front/images/supermarket.png">
                        <span class="cartCount cart-val"><?php echo count($cartData) ?></span>
                      </a>
                    </div>
                    <?php
                  }*/
                  ...
                  <?php
                     if (Request()->get('store')) {
                       ?>
                       <form method="get" class="itemSearchForm">
                          <?php
                          if (isset($requestPerameters) && is_array($requestPerameters) && !empty($requestPerameters)) {
                            foreach ($requestPerameters as $perameterKey=>$perameterValue) {
                              ?>
                              <input type="hidden" name="<?php echo $perameterKey ?>" value="<?php echo $perameterValue?>">
                              <?php
                            }
                          }
                          if (isset($openPage)) {
                          ...
                          <div class="search-block">
                           <input type="text" class="search" value="<?php echo Request()->get('item_name'); ?>" name="item_name" placeholder="Search within menu">
                           <button type="submit" class="btn-custom"><img src="<?php echo publicPath() ?>/front/images/search.png"></button>
                        </div>
                       </form>
                  <?php } } ?>
               </div>
      ...
      </header><?php /**PATH /home/k9c9adh99pg7/public_html/pza/resources/views/Include/OrderHeader.blade.php ENDPATH**/ ?>
