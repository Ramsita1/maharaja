            ...
            <div class="card">            
               <div class="card-block"> 
                  ...
                  <div class="">                          
                     ...
                     <div class="input-group row">
                        <label class="col-form-label" for="printers">printers</label>
                       
                           <?php 
                           $store_id = Auth::user()->store_id;
                           $printers = \App\Printer::where('store_id', $store_id)->get();
                           foreach ($printers as $printer) {
                              $status=0;
                                foreach($menuItem->printers as $p){
                                   if($printer->id == $p->printer_id)
                                   {
                                      $status=1;
                                   }
                                }
                              ?>
                             <div class="col-md-12">
                                    <label for="Item_for_<?php echo $printer->printer_name; ?>">
                                       <b><?php echo $printer->printer_name; ?></b>&nbsp;
                                        <input type="checkbox" class="printerCheckbox" data-showItems="show_<?php echo $printer->printer_name; ?>" name="<?php echo $menuItem->menu_item_id; ?>" <?php echo ($status == 1?'checked':'') ?> id="<?php echo $printer->id; ?>" value="1"> 
                                    </label>
                                    </div>
                        <?php 
                           }
...
<script type="text/javascript">
   jQuery(document).ready(function($) {
      $('.printerCheckbox').change(function(event) {
         var menuItemid = $(this).attr('name');
         var printerid = $(this).attr('id');
         if ($(this).is(':checked')) {
            var action = 0;
            $.ajax({
            url: '<?php echo route('insertDeletePrinter') ?>',
            type: 'GET',
            data: {printerid: printerid, menuItemid: menuItemid , action: action},
         })
...
</script><?php /**PATH C:\xampp\htdocs\MaharajaPrestonApi\resources\views/Admin/MenuItems/CreateEdit.blade.php ENDPATH**/ ?>